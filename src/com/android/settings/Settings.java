...
public class Settings extends Activity implements OnItemClickListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mLayout = getActualProperty("com.android.settings.layout");
        if (mLayout == 720) {
     ...
     * @see getProperty
     */
    private static int getActualProperty(String property) {
        int result = 0;
        boolean getProp = false;

        if (property.endsWith(ZROM_DPI_SUFFIX)) {
            ApplicationInfo appInfo = getAppInfoFromPackageName(property.substring(0, property.length()
                    - ZROM_DPI_SUFFIX.length()));
            if(appInfo != null) {
                boolean isSystemApp =
                        appInfo.sourceDir.substring(0, appInfo.sourceDir.lastIndexOf("/")).contains("system/app");
                result = Integer.parseInt(getProperty(property, getProperty(ZROM_PREFIX + (isSystemApp ?
                        "system_default_dpi" : "user_default_dpi"))));
            } else {
                getProp = true;
            }
        } else if (property.endsWith(ZROM_LAYOUT_SUFFIX)) {
            ApplicationInfo appInfo = getAppInfoFromPackageName(property.substring(0, property.length()
                    - ZROM_LAYOUT_SUFFIX.length()));
            if(appInfo != null) {
                boolean isSystemApp =
                        appInfo.sourceDir.substring(0, appInfo.sourceDir.lastIndexOf("/")).contains("system/app");
                result = Integer.parseInt(getProperty(property, getProperty(ZROM_PREFIX + (isSystemApp ?
        ...
        if (result == 0) {
            result = Integer.parseInt(property.endsWith("dpi") ? getProperty(ZROM_PREFIX + "rom_default_dpi")
                : getProperty(ZROM_PREFIX + "rom_default_layout"));
        }
