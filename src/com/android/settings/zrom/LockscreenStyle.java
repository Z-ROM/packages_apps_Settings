...
public class LockscreenStyle extends SettingsPreferenceFragment
    ...
    private void updateLockSummary() {
        ...
        } else if (value.contains("zrom_lock")) {
            resId = R.string.lockscreen_lock_icon_zrom;
    ...
    private void resizeZLock() {
        Bitmap zLock = BitmapFactory.decodeResource(getResources(), R.drawable.zrom_lock);
        if (zLock != null) {
            String path = null;
            int px = requestImageSize();
            zLock = Bitmap.createScaledBitmap(zLock, px, px, true);
            try {
                mLockImage.createNewFile();
                mLockImage.setWritable(true, false);
                File image = new File(getActivity().getFilesDir() + File.separator
                            + "zrom_lock" + System.currentTimeMillis() + ".png");
                path = image.getAbsolutePath();
                mLockImage.renameTo(image);
                FileOutputStream outPut = new FileOutputStream(image);
                zLock.compress(Bitmap.CompressFormat.PNG, 100, outPut);
                image.setReadable(true, false);
                outPut.flush();
                outPut.close();
            } catch (Exception e) {
